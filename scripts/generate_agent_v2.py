...
def generate_agent_v2(account_id):
    memo_path = f"outputs/accounts/{account_id}/v2/memo_v2.json"
    if not os.path.exists(memo_path):
        print(f"Error: Memo v2 not found for account {account_id}")
        sys.exit(1)
...
"""
    ...
    with open(f"{output_dir}/agent_v2.json", "w") as f:
        json.dump(agent_spec, f, indent=4)
        
    print(f"[{account_id}] Generated agent config v2 to {output_dir}/agent_v2.json")

if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python generate_agent_v2.py <account_id>")
        sys.exit(1)
    generate_agent_v2(sys.argv[1])
